...
def ICP2(P, Q, thresh=1e-5, max_iter=50):
    """
    Standard ICP implementation for point clouds P and Q.
    Args:
        P: Nx3 array of 3D points from set P.
        Q: Nx3 array of 3D points from set Q.
    ...
    for _ in range(max_iter):
        # Find closest points
        distances = np.linalg.norm(P_centered[:, None, :] - Q_centered[None, :, :], axis=2)
        indices = np.argmin(distances, axis=1)
        Q_closest = Q_centered[indices]
        
        # Compute SVD for rotation
        W = Q_closest.T @ P_centered
        ...
        # Transform P
        P_centered_new = (R @ P_centered.T).T + T
        if np.linalg.norm(P_centered_new - P_centered) < thresh:
            break
        P_centered = P_centered_new
    
    return R, T

def RANSAC_ICP(matched_points1, matched_points2, max_iter=300, inlier_thresh=5, icp_thresh=1e-5):
    """
    RANSAC with ICP as the model.
    Args:
        matched_points1: Nx3 array of 3D points from set 1.
        matched_points2: Nx3 array of 3D points from set 2.
        max_iter: Maximum number of RANSAC iterations.
        inlier_thresh: Distance threshold for determining inliers.
        icp_thresh: Convergence threshold for ICP.
    Returns:
        best_R: Best rotation matrix found.
        best_T: Best translation vector found.
        best_inliers: Indices of inliers.
    """
    best_inliers = []
    best_R, best_T = None, None
    n_points = matched_points1.shape[0]
    
    for _ in range(max_iter):
        # Step 1: Randomly sample points
        indices = np.random.choice(n_points, 3, replace=False)
        sample1 = matched_points1[indices]
        sample2 = matched_points2[indices]
        
        # Step 2: Apply ICP on the sampled points
        R, T = ICP2(sample1, sample2)
        
        # Step 3: Transform matched_points1 using R, T
        transformed_points = (R @ matched_points1.T).T + T
        
        # Step 4: Compute distances and count inliers
        distances = np.linalg.norm(transformed_points - matched_points2, axis=1)
    ...
    # Final step: Run ICP with all inliers for refinement
    if len(best_inliers) > 0:
        final_R, final_T = ICP2(matched_points1[best_inliers], matched_points2[best_inliers], thresh=icp_thresh)

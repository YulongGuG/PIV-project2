...
"""
IMGsmatch               = [
    [np.zeros((0, 0)) if i == j else fcv.KpMatch_comb(Desc[i], Desc[j]) for j in range(N)]
    for i in range(N)
]
...
for i in range(N):
    ...
    for j in range(N):
        counter += 1
        print(counter)
        if KpsComb[i][j].size <10:  # 如果匹配点为空
            a.append(np.zeros((0, 6)))  # 默认填充空3D点对
            b.append(None)  # 默认填充空的R
            c.append(None)  # 默认填充空的T
            d.append([])  # 默认填充空的内点索引
            continue
        Kp2d1           = KpsComb[i][j][:, 0:2]
        Kp2d2           = KpsComb[i][j][:, 2:4]
        Kp3d1           = f.GetKp3d(Kp2d1, depths[i], fls[i][0,0])
        Kp3d2           = f.GetKp3d(Kp2d2, depths[j], fls[j][0,0])
        R, T, Imatch    = f.RANSAC_ICP(Kp3d1, Kp3d2)
...
"""
'''InlierMatch             = [
    [np.zeros((0, 0)) if i == j else f.RANSAC_ICP(KpsComb[i][j][:, 1:2]) for j in range(N)]
    for i in range(N)
]'''
